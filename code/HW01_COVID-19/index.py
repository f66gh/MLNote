...
def plot_pred(dv_set, model, device, lim=35., preds=None, targets=None):
    ...
    if preds is None or targets is None:
        ...
        for x, y in dv_set:
            ...
            # 在不计算梯度的情况下进行前向传播（节省内存）
            with torch.no_grad():
                # 模型预测
                pred = model(x)
                # 将预测结果从GPU移到CPU并添加到列表
                preds.append(pred.detach().cpu())
                # 将真实值从GPU移到CPU并添加到列表
                targets.append(y.detach().cpu())
        # 将列表中的张量连接成一个张量，并转换为numpy数组
        preds = torch.cat(preds, dim=0).numpy()
        targets = torch.cat(targets, dim=0).numpy()

    # 创建5x5英寸的正方形图形
    figure(figsize=(5, 5))
    # 绘制散点图：x轴为真实值，y轴为预测值（红色，半透明）
    plt.scatter(targets, preds, c='r', alpha=0.5)
    # 绘制理想预测线（y=x，蓝色）表示完美预测的情况
...
# 数据路径配置
tr_path = 'covid.train.csv'  # 训练数据路径
...
print(f'预测完成！结果已保存到 pred.csv 文件中')
print(f'训练完成，最佳验证损失: {model_loss:.4f}')