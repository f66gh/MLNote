...
class NeuralNet(nn.Module):
    ...
    def __init__(self, input_dim):  # 输入维度
        ...
        self.net = nn.Sequential(  # 顺序容器
            nn.Linear(input_dim, 64),  # 全连接层：输入维度 -> 64
            nn.ReLU(),  # ReLU激活函数
...
target_only = True # TODO: 是否只使用40个州和2个tested_positive特征

# TODO: 如何调整这些超参数来提高模型性能？
config = {
    'n_epochs': 3000,  # 最大训练轮数
    'batch_size': 270,  # 数据加载器的小批次大小
    'optimizer': 'SGD',  # 优化算法（torch.optim中的优化器）
...
# 创建训练、验证和测试数据加载器
tr_set = prep_dataloader(tr_path, 'train', config['batch_size'], target_only=target_only)  # 训练集
dv_set = prep_dataloader(tr_path, 'dev', config['batch_size'], target_only=target_only)  # 验证集
tt_set = prep_dataloader(tt_path, 'test', config['batch_size'], target_only=target_only)  # 测试集
