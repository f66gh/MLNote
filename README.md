...
先train一个大的network（叫做Teacher Network），再做一个小的network（student Net）.

学生要逼近老师得出的结果，不论老师对错，学生也不需要根据正确的答案学。

如下图所示，老师只需要告诉学生哪些数字间有什么样的关系，例如老师说1和7长得很像，尽管学生没有看过7是什么样子，但是学生也可以辨别出7.

![image-20250918162642667](./pic/image-20250918162642667.png)

老师network也可以是多个网络ensemble起来的，可以是多个模型输出的平均值。

![image-20250918162852709](./pic/image-20250918162852709.png)

在使用知识蒸馏时，需要稍微改一下softmax的方程。T是一个超参数，需要自己调。

T比较大时，会让分布比较平滑。普通的softmax的输入数据差距比较大时，输出的差距也会很大，这样和独热向量没什么区别了。老师的作用是告知学生正确答案，以及这个输入和哪些输入比较像。

![image-20250918163309940](./pic/image-20250918163309940.png)



### Parameter Quantization 参数量化 

这张PPT介绍了**参数量化 (Parameter Quantization)**，这是一种非常重要的**模型压缩 (Model Compression)** 技术。它的核心目标是让深度学习模型变得更小、更快，以便能部署在手机、嵌入式设备等资源有限的硬件上。

用比较少的空间存储参数。例如原来是16bit存一个数值，现在是8bit存一个数值。

也可以用**权重聚类Weight cluster**.

* 我们可以将值接近的几个权重分到一类，赋一个值，然后再统一划表查。

也可以用**哈夫曼编码Huffman encoding**：常用的集群用小bit表示，用的少的集群用大bit表示。

![image-20250918184714481](./pic/image-20250918184714481.png)

### 架构设计 Architecture Design

通过优化架构减少参数量的效果。

复习一下标准CNN：输入图片有几个通道，卷积核就有几层。有几个卷积核，输出的矩阵就有几层。

如下图所示，这一层一共有72个参数

![image-20250918191227778](./pic/image-20250918191227778.png)

**Depthwise Separable Convolution 深度可分离卷积**：

第一步：**Depth Convolution 深度卷积**

一个通道对应一个卷积核，一个卷积核只考虑一个通道。

![image-20250918191740552](./pic/image-20250918191740552.png)

但是上一步中把每一个通道都是单独处理的，不同通道之间没有信息融合。

第二步：**Pointwise Convolution 逐点卷积**

逐点卷积本质上就是一次**标准卷积**，但它的特殊之处在于，所使用的卷积核 (filter) 尺寸是 `1 x 1`。

现在我们可以把这两步串起来，理解完整的**深度可分离卷积 (Depthwise Separable Convolution)**：

1. **第一步：深度卷积 (Depthwise Conv)**
   - **任务**：只负责空间维度的特征提取。
   - **方式**：用一组2D卷积核，在每个输入通道上“各扫门前雪”，独立地进行卷积。
   - **好比**：分别处理一张彩色照片的R、G、B三个通道，在R通道里找边缘，在G通道里找纹理，在B通道里找角点。但此时找到的“R边缘”、“G纹理”、“B角点”之间是互不相干的。
2. **第二步：逐点卷积 (Pointwise Conv)**
   - **任务**：只负责通道维度的信息融合。
   - **方式**：用一组 `1x1xC` 的3D卷积核，将上一步提取出的各个通道的特征进行加权组合，生成新的、更有意义的特征。
   - **好比**：现在把上一步找到的“R边缘”、“G纹理”、“B角点”这些信息点拿过来，进行线性组合，从而学习出“猫的眼睛”这种更高级的特征（可能需要一点R通道的边缘 + 很多G通道的纹理 + 一点B通道的角点）。

**结论：** “深度可分离卷积”这个名字的由来就是它将标准卷积的操作**分离 (Separable)** 成两步：一步负责**深度/空间 (Depthwise)**，一步负责**逐点/通道 (Pointwise)**。这种“分工合作”的方式，可以用远低于标准卷积的计算量和参数量，实现非常接近的性能，是现代轻量化神经网络设计的基石。

![image-20250918192949716](./pic/image-20250918192949716.png)

如下图所示，标准卷积和深度可分离卷积的计算量做除法。由于通道数一般都很大，结果把O项忽略不计，那么计算量成k*k倍数的下降。

![image-20250918193220861](./pic/image-20250918193220861.png)

### Dynamic Computation 动态计算

 网络动态调整运算量。

![image-20250918194743846](./pic/image-20250918194743846.png)

动态深度，每一个隐藏层都可以额外接一个外层，用来输出结果。当运算资源充足时就跑完所有层，当运算资源低时就跑到中间层然后输出。

![image-20250918194954848](./pic/image-20250918194954848.png)

 也可以让网络自由决定宽度（Dynamic Width）。下面是同一个网络，只是宽度不一样而已。

![image-20250918200703212](./pic/image-20250918200703212.png)

## 元学习 Meta Learning

超参数可不可以让机器自动学习？

![image-20250919110931921](./pic/image-20250919110931921.png)

你可以让机器自己学习网络架构，初始参数，学习率等。我们用Φ代表我们想让机器自动学的部分。

![image-20250919111437869](./pic/image-20250919111437869.png)

我们需要给学习的部分定义Loss方程。

![image-20250919111619713](./pic/image-20250919111619713.png)

也要与独热向量做交叉熵

![image-20250919111924855](./pic/image-20250919111924855.png)

在元学习中，我们会拿不止一个二元分类任务训练。总得loss是拿着所有任务的loss加起来得出的。

![image-20250919143601202](./pic/image-20250919143601202.png)

在元学习中，loss是用（训练任务里的）测试资料进行计算的。

下一步就是找到一个$Φ^*$使得$L(Φ)$最小。如果L不可微那就用RL等其他方法硬做。

![image-20250919144456087](./pic/image-20250919144456087.png)

先用**训练任务**训练好一个元学习**算法**，然后用**测试任务**中的**训练集**训练这个**算法的参数**，之后再用**测试任务**中的**测试集**判断学习效果。

![image-20250919145001172](./pic/image-20250919145001172.png)

ML与Meta Learning的差异

![image-20250919145651044](./pic/image-20250919145651044.png)

元学习中由于要训练算法，所以要用到不同的任务训练。而机器学习中的训练指的是训练特定任务的参数，所以只用一个就好了。

![image-20250919145842690](./pic/image-20250919145842690.png)

由于机器学习是一个任务训练特定的参数，所以需要测试集测试模型训练的效果。而元数据要用的测试任务既包含训练资料也包含测试资料。

![image-20250919150107364](./pic/image-20250919150107364.png)

ML中loss是用一个任务的测试结果算出来的，元学习中用的是所有学习任务的loss。

![image-20250919150649563](./pic/image-20250919150649563.png)

![image-20250919150822488](./pic/image-20250919150822488.png)

就像机器学习中，初始参数也可以学习。元学习中，我们也要找到一个好的初始参数，它能让模型在一个新的、具体的任务上通过少量数据就快速收敛到很好的效果。这就叫MAML。

![image-20250919151827695](./pic/image-20250919151827695.png)

在机器学习中，我们通常希望模型能快速学习新任务。一个好的模型**初始参数**（initialization，简称 "init"）至关重要，它能让模型在一个新的、具体的任务上通过少量数据就快速收敛到很好的效果。MAML和预训练都是为了找到这个“万能的”初始点。

下面这张图实在没看懂，AI也没给我解释明白

![image-20250919152315804](./../LeetCode/image-20250919152315804.png)

 最优化

学习率（λ）也可以用元学习训练出来。

![image-20250919153223570](./pic/image-20250919153223570.png)

网络结构搜索 Network Architecture Search (NAS)

![image-20250919153500755](./pic/image-20250919153500755.png)
